...
class LeaveForm(QDialog):
    def __init__(self,parent=None,leave_id=None,user=None):
        super().__init__(parent); self.leave_id=leave_id; self.user=user; self.setWindowTitle('Leave Form'); self.setFixedSize(520,460); self.setup_ui(); 
        if leave_id: self.load_existing()
    def setup_ui(self):
        layout=QVBoxLayout(); title=QLabel('Leave request'); title.setProperty('heading', True); layout.addWidget(title)
        layout.addWidget(QLabel('Type (Vacation / Sick / Others)')); self.type_edit=QLineEdit(); layout.addWidget(self.type_edit)
        ...
        self.remarks = QTextEdit(); self.remarks.setPlaceholderText('Admin remarks (optional)'); self.remarks.setFixedHeight(80); layout.addWidget(self.remarks)
    ...
    def load_existing(self):
        items=LC.list_for({'username':None,'role':'admin'})  # fetch all and find
        for l in items:
            if l.get('id')==self.leave_id:
                self.type_edit.setText(l.get('type','')); self.start.setDate(QDate.fromString(l.get('start_date'),'yyyy-MM-dd')); self.end.setDate(QDate.fromString(l.get('end_date'),'yyyy-MM-dd')); self.reason.setPlainText(l.get('reason','')); self.remarks.setPlainText(l.get('remarks','') or '')
    def on_save(self):
        ...
        self.accept()
    def get_data(self):
        return {'type': self.type_edit.text().strip(), 'start_date': self.start.date().toString('yyyy-MM-dd'), 'end_date': self.end.date().toString('yyyy-MM-dd'), 'reason': self.reason.toPlainText().strip(), 'remarks': self.remarks.toPlainText().strip()}
