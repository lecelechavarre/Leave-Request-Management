...
class LeaveForm(QDialog):
    ...
    def setup_ui(self):
        ...
        row=QHBoxLayout(); row.addWidget(QLabel('Start Date')); self.start=QDateEdit(); self.start.setCalendarPopup(True); self.start.setDate(QDate.currentDate()); row.addWidget(self.start)
        row.addItem(QSpacerItem(10,10,QSizePolicy.Policy.Expanding,QSizePolicy.Policy.Minimum)); row.addWidget(QLabel('End Date')); self.end=QDateEdit(); self.end.setCalendarPopup(True); self.end.setDate(QDate.currentDate()); row.addWidget(self.end)
        layout.addLayout(row); layout.addWidget(QLabel('Reason')); self.reason=QTextEdit(); self.reason.setFixedHeight(160); layout.addWidget(self.reason)
