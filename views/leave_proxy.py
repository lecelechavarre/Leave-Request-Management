...
class LeaveFilterProxy(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        ...
        if self.search:
            ...
            if self.search not in text:
                return False
        if self.status != 'All' and data(5) != self.status.lower():
            return False
        if self.type != 'All':
            if self.type == 'Others' and data(2) in ('vacation','sick'):
                return False
            elif self.type != 'Others' and data(2) != self.type.lower():
                return False
